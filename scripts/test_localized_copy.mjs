...
for (const label of ["建議人數", "遊戲時間", "推出時間", "恐怖指數", "燒腦程度"]) {
...
if (topics.some((topic) => !(topic.release_time === null || typeof topic.release_time === "string"))) throw new Error("Invalid release_time field");
...
console.log(JSON.stringify({ topics: topics.length, unknownDurationFallbacks: topics.filter((topic) => topic.duration === "依官網公告").length, unknownReleaseTimes: topics.filter((topic) => topic.release_time === null).length, uniquePros: new Set(topics.flatMap((topic) => topic.pros)).size, uniqueCons: new Set(topics.flatMap((topic) => topic.cons)).size, forbiddenTerms: 0, cta: true }, null, 2));
